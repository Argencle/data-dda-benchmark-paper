...
DATA = "Figures/figure2_data.csv"
OUTFIG = "Figures/Figure2.pdf"
...
def main():
    ...
    for N, ax_t, ax_s in [
        ...
        if N == 150:
            ...
            ax_t.set_yticks([50, 150, 500])
            ax_t.set_yticklabels(["50", "150", "500"])
        else:
            ax_t.set_ylabel("")
            ax_t.set_yticks([250, 500, 2500])
            ax_t.set_yticklabels(["250", "500", "2500"])
        ...
        ideal = cores / cores[0]
        ax_s.plot(
            cores[:3], ideal[:3], linestyle="--", linewidth=1.0, color="black"
        )
    ...
    # x-limits
    all_cores = np.sort(df["cores"].dropna().unique()).astype(float)
    if all_cores.size:
        xmin = all_cores.min()
        xmax = all_cores.max()
        for ax in [ax_time_150, ax_time_250, ax_speed_150, ax_speed_250]:
            ax.set_xlim(left=xmin - 0.15, right=xmax * 1.2)
