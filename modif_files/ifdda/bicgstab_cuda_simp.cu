...
#include "kernel_simp.h"

// #define TEST

// Fonctions auxiliaires pour manipuler les nombres complexes

// cuComplex dconjg(cuComplex cin);
// float cdabs(cuComplex x);
// cuComplex cdsqrt(cuComplex x);

#define LINE_SIZE 1000
typedef struct {
    cuComplex pt[LINE_SIZE];
} LINE_OF_COMPLEX;
...
void BICGSTAB_cuda(cuComplex *X, cuComplex *XI, cuComplex *XR, cuComplex *B, int lda, int nlar, int ndim,
                    int *nou, cuComplex *WRK_CPP, cuComplex *ALPHA, cuComplex *OMEGA, cuComplex *RHO,
                    float *NORM, float TOLE, int *ITNO, int MAXIT, int *STATUS, int *STEPERR) {

    cuComplex zzero = make_cuComplex(0.0f, 0.0f);
    cuComplex uncomp = make_cuComplex(1.0f, 0.0f);
    cuComplex ctmp, ctmp1, ctmp2, KAPPA, BETA, XXI, RHO0;
    float tmp, RESIDU;

    cuComplex *WRK = &WRK_CPP[-lda];
...
label100:
    ...
    *RHO = calc_scalar_product(&WRK[2 * lda], &WRK[1 * lda], ndim);
    ...
    KAPPA = cuCmulf(RHO0, *OMEGA);

    if (cuCabsf(KAPPA) == 0.0f) {
        *STATUS = -3;
        *STEPERR = 6;
        return;
    }

    BETA = cuCdivf(cuCmulf(*RHO, *ALPHA), KAPPA);
    // printf("BETA %g \n", BETA);
    // printf("RHO %g \n", *RHO);
    // printf("ALPHA %g \n", *ALPHA);
    // printf("KAPPA %g \n", KAPPA);
...
label30:
    ...
    if (cuCabsf(ctmp1) == 0.0f) {
        *STATUS = -3;
        *STEPERR = 14;
        return;
    }

    *OMEGA = cuCdivf(ctmp2, ctmp1);
    // printf("OMEGA %g \n", *OMEGA);
