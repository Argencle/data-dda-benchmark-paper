...
! 19.07.10 (BTD): * add sanity check to check NLAR
!                   suggested by Fangzhou Liu
!end history
! Copyright (C) 1993,1994,1995,1996,1997,1998,2003,2004,2006,2007,2008,
!               2010,2011,2012,2013,2019 B.T. Draine and P.J. Flatau
! This code is covered by the GNU General Public License.
!***********************************************************************
!*** diagnostic
!      write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 0 myid=',myid, &
!                                      ' cxf11(7)=',cxf11(7)          !
!      write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 0 myid=',myid, &
!                                      ' cxf12(7)=',cxf12(7)          !
!      write(0,*)'          mxn3=',mxn3
...
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 4'
!            write(0,*)'          icomp(1,1-3)=',icomp(1),icomp(1+nat), &
!                                 icomp(1+2*nat)
!***
...
!            write(0,*)'  j      cxalph(j),'
!            do j=1,nat3
!               write(0,fmt='(i6,2f10.5,a)')j,cxalph(j),' getfml_v6 ckpt 6'
!            enddo
!            write(0,*)'getfml_v6 ckpt 6'
!***
...
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 7, myid=',myid
!            write(0,*)'          mxn3=',mxn3
!            write(0,*)'           nat=',nat
!            write(0,*)'   j ix iy iz ',       &
!                      ' -------  A_x -------', &
...
!*** diagnostic
!               write(0,*)'getfml_v6 ckpt 8, myid=',myid,' CMDSOL=PETRKP'
!***
               ITERN=0
...
!*** diagnostic
!               write(0,*)'getfml_v6 ckpt 9, myid=',myid,' CMDSOL=SBIGCM'
!***
...
!*** diagnostic
!               write(0,*)'getfml_v6 ckpt 10, myid=',myid,' CMDSOL=PBCGS2'
!***
...
! ipar(12) - convergence;  ipar(12)=0 converged
               ...
               NONZERO_X=.FALSE.
!*** diagnostic
!               write(0,*)'getfml_v6 ckpt 13'
!               write(0,*)'   initialize CXPOL_TF, NAT3=',NAT3
!***

               CALL CINIT(NAT3,CMPLX(0._WP,0._WP,KIND=WP),CXPOL_TF,1)

!*** diagnostic   check initialization of cxpol_tf to zero'
!               do j=1,nat3
!                  if(.not.(abs(cxpol_tf(j)).lt.1.e-10))then
!                     write(0,*)'getfml_v6 ckpt 14 problem for j=',j, &
!                               ' cxpol_tf(j)=',cxpol_tf(j)
!                  endif
!               enddo
!***
...
!*** diagnostic
!               do j=1,nat3
!                  if(.not.(abs(cxpol_tf(j)).lt.1.e30))then
!                     write(0,*)'getfml_v6 ckpt 15 problem for j=',j, &
!                               ' cxpol_tf(j)=',cxpol_tf(j)
!                  endif
!               enddo
!***
...
!*** diagnostic
!               write(0,*)'getfml_v6 ckpt 20, myid=',myid
!               write(0,*)'          nlar=',nlar
!***
! sanity check
               IF(NLAR/=10)THEN
...
!            if(nat<301)then
!               write(0,*)'getfml_v6 ckpt 22: diagnostic for nat < 301'
!               write(0,*)'   j ix iy iz ',       &
!                         ' -------  P_x -------', &
!                         ' -------  P_y -------', &
!                         ' -------  P_z -------'
...
!***
            ...
            ELSEIF(JO==2)THEN
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 26, myid=',myid,' lace=',lace
!***
...
!*** diagnostic
!            write(0,'(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 30 myid=',myid, &
!                                        ' cxf11(7)=',cxf11(7)            !
!            write(0,*)'       NAT0=',nat0
!            write(0,*)'       X0=',X0
!***
...
!*** diagnostic
!            write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 31 myid=',myid, &
!                                            ' cxf11(7)=',cxf11(7)           !
!***
...
!*** diagnostic
!            write(0,fmt='(a,i4,a,1p2e10.3)')'getfml_v6 ckpt 32 myid=',myid, &
!                                        ' cxf12(7)=',cxf12(7)               !
!***
               CALL SCAT(AK_TF,AKS_TF,DX,EM1_TF,EM2_TF,E02,ETASCA,CMDTRQ,   &
                         CBKSCA,CSCA,CSCAG,CSCAG2,CTRQAB,CTRQSC,CXE_TF,     &
                         ...
                         NAT0,NAT03,NAVG,NSCAT,SCRRS1,SCRRS2,IXYZ0,X0)      !
!*** diagnostic
!            write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 33 myid=',myid, &
!                                            ' cxf12(7)=',cxf12(7)           !
!***
...
!*** diagnostic
!         write(0,*)'getfml_v6 ckpt 44, myid=',myid
!         do i=1,nat0
!            write(0,fmt='(i3,1p6e10.2,a)')i,cxe_tf(i),cxe_tf(i+nat0), &
!                                            cxe_tf(i+2*nat0),' cxe_tf'
!         enddo
...
!*** diagnostic
            write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 45 myid=',myid, &
                                            'cxf11(7)=',cxf11(7)            !
!***
...
!*** diagnostic
            write(0,fmt='(a,i4,a,1p2e10.3)')'getfml_v6 ckpt 46 myid=',myid, &
                                            ' cxf11(7)=',cxf11(7)           !
!***
!*** second call to timing routine
...
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 47, myid=',myid
...
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 48, myid=',myid
!***
!*** second call to timing routine
...
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 49, myid=',myid
!***
         CALL ALPHADIAG(AK_TF,BETADF,PHIDF,THETADF,CALPHA,CXALPH,CXALOF,       &
...
!*** diagnostic
!            write(0,*)'getfml_v6 ckpt 50, myid=',myid
!***
         CALL REDUCE(CXALPH,IOCC,MXN3,MXNAT,NAT,NAT0)
         CALL REDUCE(CXALOF,IOCC,MXN3,MXNAT,NAT,NAT0)
         CALL TIMEIT(' ALPHA',DTIME)
...
!*** diagnostic
!         write(0,*)'getfml_v6 ckpt 51, myid=',myid
!         do i=1,nat0
!            write(0,fmt='(i3,1p6e10.2,a)')i,cxe_tf(i),cxe_tf(i+nat0), &
!                                            cxe_tf(i+2*nat0),' cxe_tf'
!         enddo
...
!*** first call to timing routine:
         ...
         CALL TIMEIT(' SCAT',DTIME)
!*** diagnostic
            write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 52 myid=',myid, &
                                            ' cxf12(7)=',cxf12(7)           !
!***
...
!*** diagnostic
            write(0,fmt='(a,i4,a,1p2e10.2)')'getfml_v6 ckpt 53 myid=',myid, &
                                            ' cxf12(7)=',cxf12(7)           !
!***
!*** second call to timing routine
...
!*** diagnostic
!      write(0,*)'getfml_v6 ckpt 54, myid=',myid,' returning from getfml'
!***
      RETURN
