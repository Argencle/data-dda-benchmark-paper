...
! 25.04.04 (BTD): alphadiag_v4.f90
!                 Prof. Michel Devel (SupMicroTech-ENSMM Bescancon)
!                 noted a sign error in calculation of FLTRCD polarizability
...
! sanity check -- make sure icomp has sensible values
      do ia=1,nat
         do l=1,3
            if(icomp(ia,l).lt.0.or.icomp(ia,l).gt.ncomp)then
               write(0,*)'  problem for ia,l=',ia,l,' icomp(ia,l)=',icomp(ia,l)
            endif
         enddo
      enddo
!      write(0,*)'alphadiag_v3 ckpt 1.1'
!***
...
! 25.03.28 Michel Devel reported sign error in UserGuide
! 25.04.04 (BTD) corrected here and in UserGuide
